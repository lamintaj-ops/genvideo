...
def api_download(job_id, aspect):
    ...
    with jobs_lock:
        ...
        if job['status'] != 'completed':
            return "Video not ready yet", 400
        
        if 'files' not in job:
            return "File not found", 404
    
    if aspect not in ['169', '916']:
        return "Invalid video format", 400
    
    file_path = job['files'][aspect]
    
    if not Path(file_path).exists():
        return "File not found", 404
    
    filename = f"aquaverse_{'16-9' if aspect == '169' else '9-16'}_{job_id[:8]}.mp4"
...
def api_jobs():
    ...
    return jsonify([{
        'id': j['id'],
        'prompt': j['prompt'][:100] + ('...' if len(j['prompt']) > 100 else ''),
        'status': j['status'],
